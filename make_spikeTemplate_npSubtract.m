function [ singleSp_template_all, burst_template,allSpikes_template_all,Fig,bootStrap_template ] = make_spikeTemplate_npSubtract( spikeTimes,Metadata,filtSweep,deltaF,rawTimeSeries,npTimeSeries )
...
for J=1:length(fns_use)
    ...
    f=subplot(3,1,3)
    hold on
    plot((1:length(deltaF.(fn).ROI1))/sampRateIm,rawTimeSeries.(fn).ROI1)
        plot((1:length(deltaF.(fn).ROI1))/sampRateIm,npTimeSeries.(fn).ROI1)
